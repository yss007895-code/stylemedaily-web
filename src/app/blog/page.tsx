import Link from 'next/link';
import SafeImage from '@/components/SafeImage';
import type { Metadata } from 'next';
import NewsletterCTA from '@/components/NewsletterCTA';

export const metadata: Metadata = {
  title: 'Blog — Fashion Tips, Trends & Style News',
  description: 'The latest fashion tips, trend reports, and style advice for modern women. Stay updated with seasonal trends, outfit ideas, and product reviews.',
  keywords: ['fashion blog', 'style news', 'fashion trends 2026', 'outfit ideas', 'style tips'],
};

const posts = [
  {
    slug: 'quiet-luxury-guide',
    title: 'Quiet Luxury in 2026: How to Nail the "Old Money" Look on Any Budget',
    excerpt: 'The quiet luxury trend isn\'t going anywhere. Here\'s how to achieve that understated, expensive-looking aesthetic without spending a fortune — from fabric choices to color palettes.',
    date: 'Feb 18, 2026',
    time: '8 min',
    cat: 'Style',
    image: '/images/blog/quiet-luxury-guide.webp',
    link: '/blog/quiet-luxury-guide',
  },
  {
    slug: 'spring-color-trends',
    title: 'The 5 Colors That Will Dominate Spring 2026 (And How to Wear Them)',
    excerpt: 'Vanilla yellow, soft sage, powder blue, warm terracotta, and lavender are this season\'s key colors. We break down how to incorporate each into your existing wardrobe.',
    date: 'Feb 17, 2026',
    time: '6 min',
    cat: 'Trends',
    image: '/images/blog/spring-color-trends-2026.webp',
    link: '/blog/spring-color-trends',
  },
  {
    slug: 'capsule-wardrobe-mistakes',
    title: '7 Capsule Wardrobe Mistakes That Are Costing You Money',
    excerpt: 'Building a capsule wardrobe sounds simple, but most people get it wrong. From buying too many neutrals to ignoring fit, here are the mistakes to avoid.',
    date: 'Feb 16, 2026',
    time: '7 min',
    cat: 'Guide',
    image: '/images/blog/capsule-wardrobe-mistakes.webp',
    link: '/blog/capsule-wardrobe-mistakes',
  },
  {
    slug: 'nordstrom-vs-asos',
    title: 'Nordstrom vs ASOS: We Compared 9 Categories So You Don\'t Have To',
    excerpt: 'Quality, pricing, sizing, returns, sustainability — we spent weeks comparing both retailers to help you decide where your fashion budget goes further.',
    date: 'Feb 15, 2026',
    time: '11 min',
    cat: 'Comparison',
    image: '/images/blog/nordstrom-vs-asos-comparison.webp',
    link: '/compare/nordstrom-vs-asos',
  },
  {
    slug: 'jeans-buying-guide',
    title: 'We Tried 40+ Pairs of Jeans So You Don\'t Have To — Here Are the 10 Best',
    excerpt: 'From high-waisted wide legs to the perfect straight cut — our honest, tested rankings for every body type and budget in 2026.',
    date: 'Feb 14, 2026',
    time: '15 min',
    cat: 'Review',
    image: '/images/blog/best-jeans-buying-guide.webp',
    link: '/guides/how-to-style-wide-leg-jeans-like-a-pro-2026',
  },
  {
    slug: 'body-shape-dressing',
    title: 'The Visual Guide to Dressing for Your Body Shape',
    excerpt: 'Pear, apple, hourglass, rectangle, or inverted triangle — find your type and discover the specific cuts, fabrics, and silhouettes that flatter you most.',
    date: 'Feb 12, 2026',
    time: '16 min',
    cat: 'Guide',
    image: '/images/blog/body-shape-dressing-guide.webp',
    link: '/guides/how-to-rock-jumpsuits-for-any-occasion-2026',
  },
  {
    slug: 'amazon-fashion-haul',
    title: '25 Amazon Fashion Finds Under $30 That Actually Look Expensive',
    excerpt: 'From Editor\'s Choice picks to hidden gems -- every item is rated 4+ stars and looks way more expensive than it is. Our honest take after testing each one.',
    date: 'Feb 10, 2026',
    time: '12 min',
    cat: 'Finds',
    image: '/images/blog/amazon-fashion-haul-under-30.webp',
    link: '/guides/quiet-luxury-essentials-investment-pieces-2026',
  },
  {
    slug: 'spring-workwear',
    title: 'Spring Workwear Capsule: 15 Pieces, 30 Outfits',
    excerpt: 'Transition your work wardrobe into spring with these 15 versatile pieces. We mapped out 30 outfit combinations so you never have to think about getting dressed.',
    date: 'Feb 8, 2026',
    time: '12 min',
    cat: 'Workwear',
    image: '/images/blog/spring-workwear-capsule.webp',
    link: '/guides/office-siren-corporate-wear-guide-2026',
  },
  {
    slug: 'interview-outfits',
    title: 'What to Wear to Every Type of Job Interview in 2026',
    excerpt: 'Corporate, startup, creative agency, or remote — first impressions matter. The complete guide to looking professional without overdressing.',
    date: 'Feb 6, 2026',
    time: '11 min',
    cat: 'Guide',
    image: '/images/blog/interview-outfits-guide.webp',
    link: '/guides/the-ultimate-office-siren-style-guide-how-to-get-the-viral-tiktok-look',
  },
  {
    slug: 'summer-preview',
    title: 'Summer 2026 Fashion Preview: What We\'re Already Adding to Cart',
    excerpt: 'It\'s never too early to prep for summer. From linen everything to the return of the maxi dress — here\'s what we\'re buying now before it sells out.',
    date: 'Feb 4, 2026',
    time: '9 min',
    cat: 'Preview',
    image: '/images/blog/summer-2026-fashion-preview.webp',
    link: '/guides/old-money-summer-aesthetic-guide-2026',
  },
  {
    slug: 'sunglasses-face-shape',
    title: 'The Only Sunglasses Guide You Need: Best Frames for Your Face Shape',
    excerpt: 'Round, oval, square, or heart — we matched the best frame styles to every face shape, all under $50. Includes our tested top picks.',
    date: 'Feb 2, 2026',
    time: '10 min',
    cat: 'Accessories',
    image: '/images/blog/sunglasses-face-shape-guide.webp',
    link: '/guides/coquette-aesthetic-2026-ultimate-guide',
  },
  {
    slug: 'stitch-fix-review',
    title: 'Stitch Fix vs Personal Stylist: An Honest Comparison After 6 Months',
    excerpt: 'We tested Stitch Fix for 6 months and compared it to working with a personal stylist. Here\'s the real cost breakdown and which option delivered better results.',
    date: 'Jan 30, 2026',
    time: '10 min',
    cat: 'Review',
    image: '/images/blog/stitch-fix-review-after-6-months.webp',
    link: '/guides/gorpcore-womens-style-guide-2026',
  },
  {
    slug: 'quiet-luxury-essentials-2026',
    title: 'The Ultimate Quiet Luxury Essentials Guide 2026',
    excerpt: 'The five foundational pieces that make any outfit look like it cost ten times more than it did.',
    date: 'Feb 20, 2026',
    time: '12 min',
    cat: 'Editorial',
    image: '/images/blog/quiet-luxury-essentials-2026.webp',
    link: '/blog/quiet-luxury-essentials-2026',
  },
  {
    slug: 'mob-wife-glamour-guide',
    title: 'Mob Wife Glamour Aesthetic: Your Ultimate Style Guide',
    excerpt: 'How to master the bold, unapologetic glamour trend that refuses to whisper when it can roar.',
    date: 'Feb 25, 2026',
    time: '11 min',
    cat: 'Trends',
    image: '/images/blog/mob-wife-glamour-aesthetic.webp',
    link: '/blog/mob-wife-glamour-guide',
  },
  {
    slug: 'wide-leg-jeans-styling-2026',
    title: 'How to Style Wide Leg Jeans Like a Pro in 2026',
    excerpt: 'The five outfit formulas that make wide leg jeans look intentional instead of sloppy, regardless of your height.',
    date: 'Feb 28, 2026',
    time: '12 min',
    cat: 'Guide',
    image: '/images/blog/wide-leg-jeans-styling-2026.webp',
    link: '/blog/wide-leg-jeans-styling-2026',
  },
  {
    slug: 'soft-girl-aesthetic-2026',
    title: 'Soft Girl Aesthetic 2026: The Complete Styling Guide',
    excerpt: 'How to nail the romantic, pastel-drenched look without crossing into costume territory.',
    date: 'Mar 1, 2026',
    time: '11 min',
    cat: 'Trends',
    image: '/images/blog/soft-girl-aesthetic-2026.webp',
    link: '/blog/soft-girl-aesthetic-2026',
  },
  {
    slug: 'indie-sleaze-revival-2026',
    title: 'Indie Sleaze Revival 2026: How to Get the Look',
    excerpt: 'The raw, rebellious 2000s aesthetic is back with a vengeance. Your guide to nailing it without looking like a costume.',
    date: 'Mar 2, 2026',
    time: '10 min',
    cat: 'Trends',
    image: '/images/blog/indie-sleaze-revival-2026.webp',
    link: '/blog/indie-sleaze-revival-2026',
  },
  {
    slug: 'tomato-girl-summer-2026',
    title: 'Tomato Girl Summer 2026: The Definitive Guide',
    excerpt: 'The Mediterranean-inspired aesthetic that proves red is the only neutral you need this summer.',
    date: 'Mar 3, 2026',
    time: '11 min',
    cat: 'Trends',
    image: '/images/blog/tomato-girl-summer-2026.webp',
    link: '/blog/tomato-girl-summer-2026',
  },
  {
    slug: 'old-money-working-women-2026',
    title: 'Old Money Style for Working Women 2026',
    excerpt: 'How to translate old money aesthetics into a professional wardrobe that commands respect.',
    date: 'Mar 4, 2026',
    time: '12 min',
    cat: 'Workwear',
    image: '/images/blog/old-money-working-women-2026.webp',
    link: '/blog/old-money-working-women-2026',
  },
  {
    slug: 'napoleon-jacket-trend-2026',
    title: 'The Napoleon Jacket Trend: Why Military Style Is Back',
    excerpt: 'Structured shoulders, gold buttons, and commanding silhouettes. The military jacket trend is here.',
    date: 'Mar 5, 2026',
    time: '10 min',
    cat: 'Trends',
    image: '/images/blog/napoleon-jacket-trend-2026.webp',
    link: '/blog/napoleon-jacket-trend-2026',
  },
  {
    slug: 'minimalist-capsule-spring-2026',
    title: 'Minimalist Capsule Wardrobe for Spring 2026',
    excerpt: 'Fifteen pieces, zero decision fatigue. The spring capsule that covers every occasion.',
    date: 'Mar 6, 2026',
    time: '12 min',
    cat: 'Guide',
    image: '/images/blog/minimalist-capsule-spring-2026.webp',
    link: '/blog/minimalist-capsule-spring-2026',
  },
  {
    slug: 'balletcore-office-siren-2026',
    title: 'Balletcore Meets Office Siren: The Hybrid Look of 2026',
    excerpt: 'When graceful ballet aesthetics meet corporate power dressing, something unexpectedly perfect happens.',
    date: 'Mar 7, 2026',
    time: '11 min',
    cat: 'Editorial',
    image: '/images/blog/balletcore-office-siren-2026.webp',
    link: '/blog/balletcore-office-siren-2026',
  },
  {
    slug: 'coastal-cowgirl-aesthetic-2026',
    title: 'Coastal Cowgirl Aesthetic 2026: The Complete Guide',
    excerpt: 'When coastal grandmother meets western ranch, you get the most wearable hybrid trend of the year.',
    date: 'Mar 7, 2026',
    time: '11 min',
    cat: 'Trends',
    image: '/images/blog/coastal-cowgirl-aesthetic-2026.webp',
    link: '/blog/coastal-cowgirl-aesthetic-2026',
  },
].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());

export default function BlogPage() {
  const featured = posts[0];
  const rest = posts.slice(1);

  return (
    <div className="pt-8 max-w-4xl mx-auto">
      <div className="mb-12 border-b border-noir-200 pb-4">
        <h1 className="font-display text-4xl sm:text-5xl font-bold uppercase tracking-tight text-editorial-text">The Edit</h1>
        <p className="text-editorial-muted font-body mt-2">Fashion insights, honest reviews, and trend analysis</p>
      </div>

      {/* Featured Post */}
      <Link href={featured.link} className="block group mb-16">
        <div className="overflow-hidden">
          {featured.image && (
            <div className="relative h-64 sm:h-80 overflow-hidden bg-editorial-light">
              <SafeImage
                src={featured.image}
                alt={featured.title}
                fill
                sizes="(max-width: 768px) 100vw, 800px"
                className="object-cover group-hover:scale-[1.02] transition-transform duration-700"
              />
            </div>
          )}
          <div className="pt-5">
            <div className="flex items-center gap-3 text-[10px] tracking-editorial uppercase text-editorial-muted font-body mb-3">
              <span className="text-editorial-accent font-semibold">{featured.cat}</span>
              <span>{featured.date}</span>
              <span>{featured.time} read</span>
            </div>
            <h2 className="font-display text-2xl sm:text-3xl font-semibold text-editorial-text group-hover:text-editorial-accent transition-colors mb-3 leading-snug">
              {featured.title}
            </h2>
            <p className="text-editorial-muted font-body leading-relaxed">{featured.excerpt}</p>
          </div>
        </div>
      </Link>

      {/* Post Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-10">
        {rest.map(p => (
          <Link key={p.slug} href={p.link} className="block group border-t border-black/10 pt-5">
            {p.image && (
              <div className="relative h-44 overflow-hidden bg-editorial-light mb-4">
                <SafeImage
                  src={p.image}
                  alt={p.title}
                  fill
                  sizes="(max-width: 768px) 100vw, 400px"
                  className="object-cover group-hover:scale-[1.02] transition-transform duration-700"
                />
              </div>
            )}
            <div>
              <div className="flex items-center gap-2 text-[10px] tracking-editorial uppercase text-editorial-muted font-body mb-2">
                <span className="text-editorial-accent font-semibold">{p.cat}</span>
                <span>{p.date}</span>
                <span>{p.time} read</span>
              </div>
              <h3 className="font-display font-semibold text-editorial-text group-hover:text-editorial-accent transition-colors mb-2 leading-snug">
                {p.title}
              </h3>
              <p className="text-sm text-editorial-muted font-body line-clamp-2">{p.excerpt}</p>
            </div>
          </Link>
        ))}
      </div>

      {/* Newsletter CTA */}
      <div className="mt-16">
        <NewsletterCTA />
      </div>
    </div>
  );
}
